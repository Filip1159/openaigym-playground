...
class ExperienceSource:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            grouped_actions = _group_list(actions, [obs_len])

            global_ofs = 0
            for env_idx, (env, action_n) in enumerate(zip([self.env], grouped_actions)):
                if self.vectorized:
                    next_state_n, r_n, terminated, truncated, _ = env.step(action_n)
                    is_done_n = terminated or truncated
                else:
...
def _group_list(items, lens):
    ...
    :return: list of items grouped by lengths
    """
    res = []
    cur_ofs = 0
    for g_len in lens:
